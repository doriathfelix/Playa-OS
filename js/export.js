...
function printTransats(){
  ...
  // N&B : gris différents par service, bordures noires
  const C = {
    s1:   {bg:'#FFFFFF', bd:'#1A1A1A', tx:'#1A1A1A', lbl:'S1',   bw:1.5},
  ...
  function slotCell(s, flex){
    ...
    const f2 = flex >= 6 ? '9.5pt': flex >= 4 ? '8.5pt' : flex >= 2 ? '7pt' : '6pt';
    // Badge : fond noir texte blanc pour lisibilité N&B
    const bdgBg = c.lbl==='S1' ? '#555' : '#111';
    return `<div style="flex:${flex};padding:1px;min-width:0">
  ...
  function salonCell(id, flex){
    const r=salonMap[id], sl=SALON_SLOTS.find(s=>s.id===id);
    if(r){
      const c=cFor(r);
      const bdgBg=c.lbl==='S1'?'#555':'#111';
      return `<div style="flex:${flex};padding:1px;min-width:0">
    ...
    return `<div style="flex:${flex};padding:1px;min-width:0">
      <div style="height:100%;background:#F7F7F7;border:1px solid #CCCCCC;border-radius:7px;display:flex;align-items:center;justify-content:center">
        <div style="font-size:5.5pt;color:#AAAAAA;font-weight:600;text-align:center">${sl?sl.lbl:id}</div>
      </div>
    </div>`;
  }
  ...
  function renderRow100(){
    ...
    const mHtml=`<div style="display:flex;flex:6;height:100%">
      ...
      <div style="flex:2;padding:1px"><div style="height:100%;background:#F7F7F7;border:1px solid #CCCCCC;border-radius:7px"></div></div>
    </div>`;
    const dHtml=`<div style="display:flex;flex:7;height:100%">
      ${salonCell(1003,2)}${salonCell(1004,2)}
      <div style="flex:3;padding:1px"><div style="height:100%;background:#F7F7F7;border:1px solid #CCCCCC;border-radius:7px"></div></div>
    </div>`;
    return `<div class="row">
      <div class="row-lbl"><div class="rn">100</div><div class="rs">Resto</div></div>
  ...
  function renderDataRow(rb, sea){
    ...
    return `<div class="row">
      <div class="row-lbl"${sea?' style="border-right:3px solid #333"':''}>
        <div class="rn">${rb}</div>
        <div class="rs">${sea?'⌊Mer⌋':''}</div>
      </div>
...
@media print { .no-print { display:none !important; } }
.btn { padding:5px 14px; background:#111; color:#fff; border:none; border-radius:6px;
  font-size:12px; font-weight:700; cursor:pointer; font-family:inherit; margin-right:5px; }
.hd { display:flex; justify-content:space-between; align-items:center;
  padding-bottom:5px; border-bottom:2.5px solid #111; margin-bottom:4px; }
.date { font-size:15pt; font-weight:700; letter-spacing:-.02em; }
.stats { display:flex; gap:6px; align-items:center; }
.stat { border-radius:6px; padding:3px 9px; text-align:center;
  border:1.5px solid #333; }
.stat-n { font-size:14pt; font-weight:700; line-height:1; }
.stat-l { font-size:5.5pt; font-weight:700; text-transform:uppercase; letter-spacing:.07em; opacity:.6; margin-top:1px; }
.pills { display:flex; gap:3px; }
.pill { font-size:8pt; font-weight:800; padding:3px 9px; border-radius:10px;
  border:1.5px solid #333; background:#fff; color:#111; }
.bar { color:#fff; background:#333; border-radius:5px; text-align:center;
...
.grid { display:grid; grid-template-rows:repeat(5,1fr);
  height:148mm; border:2px solid #333; border-radius:8px; overflow:hidden; }
.row { display:flex; align-items:stretch; border-bottom:1px solid #CCCCCC; overflow:hidden; }
.row:last-child { border-bottom:none; }
.row-lbl { width:38px; flex-shrink:0; display:flex; flex-direction:column;
  align-items:center; justify-content:center;
  border-right:2px solid #333; padding:2px; background:#EEEEEE; }
.rn { font-size:9pt; font-weight:800; color:#111; line-height:1; }
.rs { font-size:5pt; font-weight:600; color:#666; margin-top:1px; }
.sep { width:4px; flex-shrink:0; background:#EEEEEE;
  border-left:1px dashed #AAAAAA; border-right:1px dashed #AAAAAA; }
/* Légende en bas à droite de l'en-tête */
.legend { display:flex; gap:5px; align-items:center; flex-wrap:wrap; }
.leg { display:flex; align-items:center; gap:3px; font-size:7pt; font-weight:600; color:#333; }
.leg-box { width:14px; height:10px; border-radius:2px; border:1.5px solid #333; }
</style></head><body>
<div class="no-print">
  <button class="btn" onclick="window.print()">⎙ Imprimer</button>
...
<div class="hd">
  <div>
    ...
    <div class="legend" style="margin-top:3px">
      <div class="leg"><div class="leg-box" style="background:#fff"></div>S1</div>
      <div class="leg"><div class="leg-box" style="background:#E0E0E0"></div>S2</div>
      <div class="leg"><div class="leg-box" style="background:#B0B0B0"></div>Soir</div>
      <div class="leg"><div class="leg-box" style="background:#fff;border-width:2.5px"></div>RT</div>
    </div>
  </div>
  <div class="stats">
    <div class="stat"><div class="stat-n">${nbSlots}</div><div class="stat-l">transats</div></div>
    <div class="stat"><div class="stat-n">${nbRT}</div><div class="stat-l">RT</div></div>
    <div class="pills">
      ${nbS1   ?`<span class="pill">S1 · ${nbS1}</span>`  :''}
      ${nbS2   ?`<span class="pill" style="background:#E0E0E0">S2 · ${nbS2}</span>`  :''}
      ${nbSoir ?`<span class="pill" style="background:#B0B0B0">Soir · ${nbSoir}</span>`:''}
    </div>
    ${unplcd.length?`<div class="stat" style="border-color:#333"><div class="stat-n">⚠${unplcd.length}</div><div class="stat-l">non placés</div></div>`:''}
  </div>
</div>
<div class="bar">▲ RESTAURANT</div>
<div class="grid">
  ${renderRow100()}
  ${TR_ROWS.filter(r=>r.id!==100).map(r=>renderDataRow(r.id,r.sea)).join('')}
</div>
<div class="bar">〰 MER MÉDITERRANÉE 〰</div>
${unplacedHtml}
</body></html>`;
