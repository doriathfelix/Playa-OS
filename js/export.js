...
function printTransats(){
  ...
  const nbS1    = placed.filter(r=>r.svc==='s1'  && !r.repas_transat).reduce((s,r)=>s+(r.slot>=1001?2:(r.tr||1)),0);
  const nbS2    = placed.filter(r=>r.svc==='s2'  && !r.repas_transat).reduce((s,r)=>s+(r.slot>=1001?2:(r.tr||1)),0);
  const nbSoir  = placed.filter(r=>r.svc==='soir'&& !r.repas_transat).reduce((s,r)=>s+(r.slot>=1001?2:(r.tr||1)),0);

  // Couleurs exactes de l'app (tokens.css)
  const C = {
    s1:   {bg:'#EDF7F1', bd:'#1A7A3E', tx:'#0D5C2E', lbl:'S1'},
    s2:   {bg:'#FEF6E8', bd:'#C47A0A', tx:'#8A5200', lbl:'S2'},
    soir: {bg:'#F0EDF9', bd:'#6B48C8', tx:'#3E2590', lbl:'Soir'},
    rt:   {bg:'#E8F4FD', bd:'#0284C7', tx:'#0A6EA6', lbl:'RT'},
    bed:  {bg:'#FFFBEB', bd:'#D97706', tx:'#92400E', lbl:'BED'},
  };
  ...
  // Labels visuels comme dans l'app
  function slotLabel(slot){
    const BED_LBL = {220:'219', 221:'220', 222:'221'};
    if(BED_SLOTS.includes(slot)) return BED_LBL[slot] || String(slot);
  ...
  // Cellule d'un slot (vide ou occupé)
  function slotCell(s, flex){
    const resa  = slotMap[s];
    const isBed = BED_SLOTS.includes(s);
    const lbl   = slotLabel(s);
    if(!resa){
      const bg = isBed ? C.bed.bg : '#F9FAFB';
      const bc = isBed ? C.bed.bd : '#E2E0DA';
      const tx = isBed ? C.bed.tx : '#C8C6C0';
      return `<div style="flex:${flex};padding:1px;min-width:0">
        <div style="height:100%;background:${bg};border:1px solid ${bc};border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden;gap:1px">
          ${isBed ? `<div style="font-size:5pt;font-weight:800;color:${tx}">BED</div>` : ''}
          <div style="font-size:5.5pt;font-weight:600;color:${tx}">${lbl}</div>
        </div>
      </div>`;
    }
    const c  = cFor(resa);
    const fs = flex >= 6 ? '11pt' : flex >= 4 ? '9.5pt' : flex >= 2 ? '8pt' : '6.5pt';
    const f2 = flex >= 6 ? '10pt' : flex >= 4 ? '8.5pt' : flex >= 2 ? '7pt' : '6pt';
    return `<div style="flex:${flex};padding:1px;min-width:0">
      <div style="height:100%;background:${c.bg};border:1.5px solid ${c.bd};border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:2px;overflow:hidden;gap:1px">
        ...
        <span style="font-size:4.5pt;font-weight:800;background:${c.bd};color:#fff;padding:0 3px;border-radius:3px;line-height:1.4">${c.lbl}</span>
      </div>
    </div>`;
  }
  ...
  function salonCell(id, flex){
    ...
    if(r){
      ...
      return `<div style="flex:${flex};padding:1px;min-width:0">
        <div style="height:100%;background:${c.bg};border:1.5px solid ${c.bd};border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:2px;overflow:hidden;gap:1px">
          <div style="font-size:8pt;font-weight:700;color:${c.tx};text-align:center;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">${r.name}</div>
    ...
    return `<div style="flex:${flex};padding:1px;min-width:0">
      <div style="height:100%;background:#F9FAFB;border:1px solid #E2E0DA;border-radius:8px;display:flex;align-items:center;justify-content:center">
        <div style="font-size:5.5pt;color:#C8C6C0;font-weight:600;text-align:center">${sl?sl.lbl:id}</div>
      </div>
    </div>`;
  }
  ...
  function renderRow100(){
    ...
    const mHtml = `<div style="display:flex;flex:6;height:100%">
      ...
      <div style="flex:2;padding:1px"><div style="height:100%;background:#F9FAFB;border:1px solid #F0EFE9;border-radius:8px"></div></div>
    </div>`;
    const dHtml = `<div style="display:flex;flex:7;height:100%">
      ${salonCell(1003,2)}${salonCell(1004,2)}
      <div style="flex:3;padding:1px"><div style="height:100%;background:#F9FAFB;border:1px solid #F0EFE9;border-radius:8px"></div></div>
    </div>`;
    return `<div class="row">
      <div class="row-lbl"><div class="row-num">100</div><div class="row-sub">Resto</div></div>
  ...
  function renderDataRow(rb, sea){
    ...
    return `<div class="row">
      <div class="row-lbl" style="${sea?'background:#EDF7F6':''}">
        <div class="row-num" style="color:${sea?'#0D7D6A':'#3A3830'}">${rb}</div>
        ${sea?`<div class="row-sub" style="color:#0D9488">Mer</div>`:'<div class="row-sub">&nbsp;</div>'}
      </div>
  ...
  let unplacedHtml = '';
  if(unplcd.length){
    unplacedHtml = `<div style="background:#FFFBEB;border:1px solid #FCD34D;border-radius:6px;padding:4px 10px;margin-top:4px">
      <span style="font-size:8pt;font-weight:800;color:#92400E">⚠ Non placés : </span>
      ${unplcd.map(r=>{const c=cFor(r);return `<span style="font-size:8pt;font-weight:700;color:${c.tx};margin-right:8px">${r.name} ⛱${r.tr||1} <span style="background:${c.bd};color:#fff;font-size:5pt;font-weight:800;padding:0 3px;border-radius:3px">${c.lbl}</span></span>`;}).join('')}
    </div>`;
  }
...
* { box-sizing:border-box; margin:0; padding:0; }
body { font-family:'DM Sans','Helvetica Neue',Arial,sans-serif; background:#fff; color:#18181A;
  -webkit-print-color-adjust:exact; print-color-adjust:exact; }
.no-print { padding:7px 0 5px }
@media print { .no-print { display:none !important; } }
.btn { padding:5px 14px; background:#18181A; color:#fff; border:none; border-radius:6px;
  font-size:12px; font-weight:700; cursor:pointer; font-family:inherit; margin-right:5px; }
.hd { display:flex; justify-content:space-between; align-items:center;
  padding-bottom:5px; border-bottom:2px solid #18181A; margin-bottom:4px; }
.date { font-size:15pt; font-weight:700; letter-spacing:-.02em; }
.stats { display:flex; gap:5px; align-items:center; }
.stat { border-radius:7px; padding:2px 9px; text-align:center; }
.stat-n { font-size:14pt; font-weight:700; line-height:1; }
.stat-l { font-size:5.5pt; font-weight:700; text-transform:uppercase; letter-spacing:.07em; opacity:.7; margin-top:1px; }
.pills { display:flex; gap:3px; }
.pill { font-size:8pt; font-weight:700; padding:3px 8px; border-radius:10px; color:#fff; }
.bar { color:#fff; border-radius:6px; text-align:center; font-size:7pt; font-weight:800;
  letter-spacing:.12em; text-transform:uppercase; padding:3px 0; margin:3px 0; }
.grid { display:grid; grid-template-rows:repeat(5,1fr);
  height:148mm; border:1px solid #E2E0DA; border-radius:10px; overflow:hidden; }
.row { display:flex; align-items:stretch; border-bottom:1px solid #E2E0DA; overflow:hidden; }
.row:last-child { border-bottom:none; }
.row-lbl { width:38px; flex-shrink:0; display:flex; flex-direction:column;
  align-items:center; justify-content:center; border-right:1px solid #E2E0DA;
  padding:2px; background:#F4F3EF; }
.row-num { font-size:9pt; font-weight:700; color:#3A3830; line-height:1; }
.row-sub { font-size:5pt; font-weight:600; color:#A8A59D; margin-top:1px; }
.sep { width:4px; flex-shrink:0; background:#F4F3EF;
  border-left:1px dashed #E2E0DA; border-right:1px dashed #E2E0DA; }
</style></head><body>
<div class="no-print">
  <button class="btn" onclick="window.print()">⎙ Imprimer</button>
  <button class="btn" style="background:#3C3C43" onclick="window.close()">✕ Fermer</button>
...
<div class="hd">
  ...
  <div class="stats">
    <div class="stat" style="background:#EDF7F1"><div class="stat-n" style="color:#0D5C2E">${nbSlots}</div><div class="stat-l" style="color:#0D5C2E">transats</div></div>
    <div class="stat" style="background:#E8F4FD"><div class="stat-n" style="color:#0A6EA6">${nbRT}</div><div class="stat-l" style="color:#0A6EA6">RT</div></div>
    <div class="pills">
      ${nbS1   ? `<span class="pill" style="background:#1A7A3E">S1 · ${nbS1}</span>`   : ''}
      ${nbS2   ? `<span class="pill" style="background:#C47A0A">S2 · ${nbS2}</span>`   : ''}
      ${nbSoir ? `<span class="pill" style="background:#6B48C8">Soir · ${nbSoir}</span>` : ''}
    </div>
...
<div class="bar" style="background:linear-gradient(90deg,#0284C7,#0D9488)">〰 MER MÉDITERRANÉE 〰</div>
${unplacedHtml}
</body></html>`;
