  ...
  sortedIds.forEach(id => {
    ...
    if(fg){
      if(!rendered.has(fg.tids[0])){
        ...
        fg.tids.forEach((tid, idx) => {
          rows.push({ label:'T'+tid, resa: idx===0 ? (r||null) : null, salon:false,
            fusionResa: r||null, fusionIdx: idx, fusionTotal: fg.tids.length });
        });
...
function printTransats(){
  ...
  const C = {
    s1:   {bg:'#FFFFFF', tx:'#111', lbl:'S1',   bw:'0.5pt'},
    s2:   {bg:'#FFFFFF', tx:'#111', lbl:'S2',   bw:'0.5pt'},
    soir: {bg:'#FFFFFF', tx:'#111', lbl:'Soir', bw:'0.5pt'},
    rt:   {bg:'#FFFFFF', tx:'#111', lbl:'RT',   bw:'0.5pt'},
  };
  ...
  function emptySlot(slot, gridRow){
    ...
    return `<div style="grid-column:${sCol(slot)};grid-row:${gridRow};margin:0.8mm;background:#EBEBEB;border:0.5pt solid #AAAAAA;border-radius:1.5mm;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden">
      ${isBed?`<div style="font-size:4.5pt;font-weight:800;color:#999;line-height:1">BED</div>`:''}
      <div style="font-size:4.5pt;color:#999;font-weight:500;line-height:1">${slotLbl(slot)}</div>
    </div>`;
  }
  ...
  function resaBlock(resa, cMin, cMax, rowMin, rowMax){
    ...
    if(nRows===1){
      slotOutlines=Array.from({length:nCols},(_,i)=>{
        ...
        return `<div style="position:absolute;left:${lPct}%;top:5%;width:${wPct}%;height:90%;border:0.5pt solid #AAAAAA;border-radius:1mm;pointer-events:none"></div>`;
      }).join('');
    }
    ...
    return `<div style="grid-column:${cMin}/${cMax+1};grid-row:${rowMin}/${rowMax+1};margin:0.8mm;position:relative;overflow:hidden;border-radius:1.5mm;background:${c.bg};border:${c.bw} solid #666">
      ${slotOutlines}
      <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0.8mm;padding:2mm 1.5mm;z-index:1">
        ...
        <div style="font-size:${fsLbl};font-weight:900;color:#fff;line-height:1;background:#111;border-radius:0.8mm;padding:0.3mm 1.5mm">${c.lbl}</div>
      </div>
    </div>`;
  }
  ...
  function salonCell(salon, gridRow){
    ...
    if(r){
      ...
      return `<div style="grid-column:${gc};grid-row:${gridRow};margin:0.8mm;position:relative;overflow:hidden;border-radius:1.5mm;background:${c.bg};border:${c.bw} solid #666">
        ${slotOutlines}
    ...
    return `<div style="grid-column:${gc};grid-row:${gridRow};margin:0.8mm;background:#EBEBEB;border:0.5pt solid #AAAAAA;border-radius:1.5mm;display:flex;align-items:center;justify-content:center">
      <div style="font-size:5pt;color:#999;font-weight:600;text-align:center;line-height:1.4">${salon.lbl}</div>
    </div>`;
  }
...
.grid{flex:1;min-height:0;display:grid;grid-template-columns:${LBL}mm repeat(7,${W}mm) ${ALLEY}mm repeat(6,${W}mm) ${ALLEY}mm repeat(8,${W}mm);grid-template-rows:15mm repeat(4,1fr);column-gap:${GAP}mm;row-gap:2mm}
...
<div class="sea">MER MEDITERRANEE</div>
${unplacedHtml}
</body></html>`;
...
function printServiceSheet() {
  ...
  const TABLE_ORDER = [
    ...
    {id:14,lbl:'14'},{id:16,lbl:'16'},{id:17,lbl:'17'},
