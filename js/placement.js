...
// qu'on peut ajouter en bout de table selon l'espace disponible).
// ⚠ Jamais : 7+8 (pas fusionnables), 12/13/14 (terrasse 2 séparée)
// Ordre : de la plus petite à la plus grande → algo prend la première qui couvre le PAX
const FUSION_COMBOS = [
  // ── Salle intérieure ──
  // T1(3) + T2(2) + 1 chaise bout T2 (espace libre côté T3) = 6
  {tids:[1,2],       hi:6},
  // T5(6) + T6(2) = 8 — T6 fait office d'extension
  {tids:[5,6],       hi:8},
  // T9(3) + T10(5) = 8
  {tids:[9,10],      hi:8},
  // T10(5) + T11(5) = 10
  {tids:[10,11],     hi:10},
  // T1+T2+T3 = 3+2+3 + 1 chaise bout = 9
  {tids:[1,2,3],     hi:9},
  // T9+T10+T11 = 3+5+5 = 13
  {tids:[9,10,11],   hi:13},
  // T1+T2+T3+T4 = 3+2+3+5 + 2 chaises = 15 (grand groupe salle)
  {tids:[1,2,3,4],   hi:15},
  // ── Tables hautes extérieur ──
  // T27(3)+T28(2) = 5
  {tids:[27,28],     hi:5},
  // T28(2)+T29(2) = 4
  {tids:[28,29],     hi:4},
...
function findBestFusion(pax, used){
  for(const combo of FUSION_COMBOS){
    if(combo.tids.some(tid => used.has(tid))) continue;
    if(combo.hi >= pax) return combo;
  }
  return null;
}
...
function autoTables(resas, skipped=0){
  ...
  const existingFusions = (fused[tk()]||[]).map(g => {
    ...
    const totalHi  = g.tids.reduce((s, tid) => s + (TABLE_DATA[tid]?.hi || 0), 0);
