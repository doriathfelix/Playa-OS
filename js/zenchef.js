...
function zcToResa(b){
  ...
  const snLow = shiftName.toLowerCase().replace(/[\s_-]/g,''); // normalise "soir 1" → "soir1"
  const isSoirShift = snLow.includes('soir') || snLow.includes('diner') || snLow.includes('dîner')
    ...
    || snLow.includes('service2') || snLow.includes('2emesvc') // cas "service 2" du soir
    || /soir[12]?$/.test(snLow);
  let svc = 's1';
  ...
  // LOG diagnostic — visible dans la console navigateur pour débugger les classements
  if(h >= 16 || isSoirShift)
    console.log(`[ZC svc] ${name} | time:${b.time} | shift:"${shiftName}" | svc→${svc}`);
